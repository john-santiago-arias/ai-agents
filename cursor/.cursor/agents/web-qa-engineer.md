...
- **Never commit or push without explicit QA confirmation.** (This agent is the intended exception to the repo's "agents never commit" rule; the exception is _only_ this confirmed, branch-scoped flow.)
...
- Playwright config, fixtures + POM, spec conventions → `.cursor/skills/playwright-e2e-stack/SKILL.md`
- Shared testing conventions (AAA, `data-testid`) → `.cursor/skills/shared-patterns/testing.md`
