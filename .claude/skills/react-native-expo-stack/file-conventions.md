...
Component declaration, component body order, import order, and the 300-line limit are universal rules — see `.claude/rules/code-style.md` and `.claude/rules/semantics-order.md`. RN-specific hooks slot into the same body order (`useNavigation`/`useIsFocused` as library hooks, `onPress` handlers before effects).
